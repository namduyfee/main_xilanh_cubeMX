...
void tu_dong_init(void) {
	...
	FEE_RTOS_struct.TuDong.khoi_tao_an_toan = 10; 
	FEE_RTOS_struct.TuDong.tha_lua = 38; 																									// tha lua vao kho 
	FEE_RTOS_struct.TuDong.gap_lua_len = 17; 																						// vi tri tren lo lua 
	FEE_RTOS_struct.TuDong.di_tha_lua = 7; 																							// di tha lua 
	FEE_RTOS_struct.TuDong.tranh_cam_bien = 36; 																			// tranh cham sw, cam bien 
//	FEE_struct.TuDong.tranh_cham_lua = 26; 
	FEE_RTOS_struct.TuDong.kep_lua = 2; 
	for(int i = 0 ; i < 20; i++) {
...
void trang_thai_init(void) {
	...
	FEE_RTOS_struct.TrangThai.count_suon_len_1 = 0; 
	FEE_RTOS_struct.TrangThai.count_suon_len_2 = 0; 
	FEE_RTOS_struct.TrangThai.luu_count_suon_len_1 = 0; 
	FEE_RTOS_struct.TrangThai.luu_count_suon_len_2 = 0; 
